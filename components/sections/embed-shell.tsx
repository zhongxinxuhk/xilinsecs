  ...
  return (
    <section className="site-shell">
      <div className="rainbow-panel overflow-hidden">
        <div className="flex flex-col items-start justify-between gap-4 border-b border-white/80 bg-white/70 px-4 py-4 text-slate-950 dark:text-slate-50 sm:flex-row sm:items-center sm:px-6">
          <div className="min-w-0">
            <p className="text-xs uppercase tracking-[0.16em] text-blue-700">Embedded workspace</p>
            <h2 className="mt-2 text-lg font-semibold text-slate-950 dark:text-slate-50">{title}</h2>
          </div>
          {note ? (
            <div className="hidden max-w-sm items-start gap-2 rounded-2xl border border-blue-100 bg-white/85 px-4 py-2 text-xs leading-6 text-slate-600 dark:text-slate-400 md:flex">
          ...
          {!loaded && visible ? (
            <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/55 p-4 backdrop-blur-sm">
              <div className="w-[min(360px,92%)] rounded-[24px] border border-white/80 bg-white/92 p-5 text-center shadow-glass sm:rounded-[28px] sm:p-8">
                ...
                <h3 className="mt-5 text-lg font-semibold text-slate-950 dark:text-slate-50">正在载入外部表单</h3>
