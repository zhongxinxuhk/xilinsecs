...
export default function StatGrid({ stats }: StatGridProps) {
  return (
    <div className="grid min-w-0 gap-4 md:grid-cols-3">
      {stats.map((stat) => (
        <article key={stat.label} className="glass-card p-5 text-slate-950 dark:text-slate-50">
          ...
          <p className="mt-3 text-sm leading-7 text-slate-600 dark:text-slate-400">{stat.detail}</p>
        </article>
      ))}
    </div>
  );
